...
```{r, message=FALSE, warning=FALSE, fig.align='center', fig.height=5, fig.width=7, fig.cap="Model Diagnostics."}
...
As we can see from the plot that the variance assumption as well as normality assumptions are not a suspect for the select interactive model. To further validate the results for Normality assumption we can do a Shapiro-Wilk test.
```{r, message = FALSE, warning=FALSE}
...
As we can see that the **`r shapiro.test(resid(lm_housing_interac_select))$p.value`** is less than **$\alpha = 0.05$** hence we Reject the null hypothesis and say that Normality is not a suspect.
