...
plot_scatterplot(plot_data, by = "SalePrice", ggtheme = gtheme, nrow = 3L, ncol = 4L,)
```
...
Looking at the data and selected variables based on our analysis we think that there should be some good interactions between variables like `HouseStyle`, `LotShape`, `LotConfig`, `BedroomAbvGr`, `TotRmsAbvGrd`, `GrLivArea`, `GarageCars`, `MSZoning`, `X1stFlrSF`, `X2ndFlrSF`, `YearBuilt` and `YearRemodAdd` with certain variables. We will explore these interactions to build the initial interaction model and then reduce the model using backward AIC.
  ...
  We will look at some specific interaction to evaluate if they contribute to the model or not. Specifically:
  - `LotShape` interaction with `X1stFlrSF`, `X2ndFlrSF`, `HouseStyle`, `GrLivArea`
  - `GrLivArea` interaction with `X1stFlrSF`, `X2ndFlrSF`, `HouseStyle`, `GarageCars`
  - `BedroomAbvGr` interaction with `X1stFlrSF`, `X2ndFlrSF`, `HouseStyle`, `YearBuilt`, `YearRemodAdd`
  - `YearBuilt` interaction with `X1stFlrSF`, `X2ndFlrSF`, `HouseStyle`, `YearRemodAdd`
...
summary(lm_housing_interac_select)
```
