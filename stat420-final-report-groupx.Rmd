...
```
Taking a threshold of `0.6` for the highly correlated variables of numeric type we can see that the columns we can select are: **`r unique(selected_variables)`**. We can validate that the variables have a good linear relationship with `SalePrice` by plotting as scatter plot.

```{r}
plot_data = housing_data[, c("SalePrice", unique(selected_variables))]
...
prop.table(table(all_categories))
