...
```
Looking at the histogram we can see that the price of the houses is Skewed to the left. A lot of the time doing a log transformation on the response helow with model building. That is we can do a log transform of the `SalePrice` and check if the values follow a normal distribution and hence helping us build a linear model.
```{r, message = FALSE, warning = FALSE, fig.align='center'}
ggplot(housing_data, aes(x = SalePrice)) +
  geom_histogram(bins=30) +
  labs(title = "Sale Price Distribution", 
