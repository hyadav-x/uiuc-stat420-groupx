...
training_data = housing_data %>% dplyr::sample_frac(0.80)
test_data = dplyr::anti_join(housing_data, training_data, by = 'Id')
...
plot_correlation(training_data, type = "continuous", ggtheme = gtheme)
```
...
numerical_cols = subset(training_data, select = -c(SalePrice)) %>% select_if(is.numeric)
...
for (i in 1:nrow(high_cor_pairs)) {
  ...
  selected_variables = c(selected_variables, var1)
}
```
Taking a threshold of `0.6` for the highly correlated variables of numeric type we can see that the columns we can select are: **`r unique(selected_variables)`**. We can validate that the variables have a good linear relationship with `SalePrice` by plotting as scatterplot.
```{r}
plot_data = training_data[, c("SalePrice", unique(selected_variables))]
plot_scatterplot(plot_data, by = "SalePrice", ggtheme = gtheme)
```


#### Categorical Variables
For the categorical variables we will check and analyze how many of the variables have better coverage of the data for different levels. Looking at the bar plot for categorical variables in housing data above (section) we can see that a lot of categorical variables have issue as wither most ofthe data are in the same category or there a very small sample of the data for a given category.

```{r}
categorical_cols = training_data %>% select_if(is.factor)
all_categories = training_data[, names(categorical_cols)]

prop.table(table(all_categories$RoofMatl))

```
We should be able to remove the categories that have a very small footprint in the data without any issue as we will get a biased result if we use the value in model building.
